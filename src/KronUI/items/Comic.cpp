...
void Comic::touchEvent(QTouchEvent* event)
{
    ...
    const int numTouchPoints = event->touchPoints().size();
    const auto firstTouchPoint = event->touchPoints().at(0);
    ...
    const Qt::TouchPointStates touchPointStates = event->touchPointStates();
    ...
    else if (pinchInProgress_)
    {
        ...
        const auto secondTouchPoint = event->touchPoints().at(1);
