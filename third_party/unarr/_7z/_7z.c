...
static bool _7z_parse_entry(ar_archive *ar, off64_t offset)
{
    ...
    ar->entry_size_uncompressed = SzArEx_GetFileSize(&_7z->data, offset);
//    ar->entry_filetime = _7z->data.MTime.Defs[offset] ?
//                ((time64_t)_7z->data.MTime.Vals[offset].Low |
//                 (time64_t)_7z->data.MTime.Vals[offset].High << 32) : 0;
